    ...
    <nav className="flex h-[80px] items-center justify-between mx-auto max-w-[1400px] px-5 md:px-14">
          ...
          <span className="font-black text-xl flex items-center">
            {Name.split("").map((letter, index) => {
              return (
                <span
                  key={index}
                  className={`hover:text-primary text-4xl font-bold  hover:-mt-2 transition-all duration-500 hover:duration-100 click:goodbyeLetterAnim ${Scr.className} ${index + 1 === 5?'text-primary':"text-foreground"}`}
                >
                  {letter}
                </span>
              );
      ...
      <ul className="flex items-center space-x-10 max-md:hidden">
        ...
        <li className=" flex items-center gap-2">
          ...
          <img className=" text-primary w-[24px] h-[24px] cursor-pointer"  alt="srccode" src="/icons/github.svg"/>
